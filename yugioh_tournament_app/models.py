...
class Deck(models.Model):
    ...
    main_deck = models.PositiveIntegerField(
        validators=[
            MinValueValidator(MIN_MAIN_DECK_SIZE),
            MaxValueValidator(AMMOUNT_OF_CARDS_PER_DECK_TYPE["main_deck"]),
        ]
    )
    side_deck = models.PositiveIntegerField(
        validators=[MaxValueValidator(AMMOUNT_OF_CARDS_PER_DECK_TYPE["side_deck"])]
    )
    extra_deck = models.PositiveIntegerField(
        validators=[MaxValueValidator(AMMOUNT_OF_CARDS_PER_DECK_TYPE["extra_deck"])]
    )
    ...
    def insert_deck(
        deck_name, main_deck, side_deck, extra_deck, owner, archtype="Mixto"
    ):
        d = Deck(
            deck_name=deck_name,
            main_deck=main_deck,
...
class Tournament(models.Model):
    ...
    champion = models.OneToOneField(
        Player, on_delete=models.RESTRICT, blank=True, null=True, default=None
    )
...
class TournamentParticipant(models.Model):  # TEST PENDING
    ...
    def check_existence(player_id, deck_id, tournament_id):  # TEST PENDING
        ...
        if not Player.objects.filter(pk=player_id).exists():
            raise ValueError(
                f"There is no element with id=={player_id} in the table Player."
            )
        if not Tournament.objects.filter(pk=tournament_id).exists():
            raise ValueError(
                f"There is no element with id=={tournament_id} in the table Tournament."
            )
    ...
    def inscription_in_time_validator(tournament, inscription_date):
        if inscription_date > tournament.start_datetime:
            raise ValidationError(
                f"This tournament already started, no new participant can be added"
            )

    def check_validations(tournament, player, deck, inscription_date):
        TournamentParticipant.check_existence(
            tournament_id=tournament.id, player_id=player.id, deck_id=deck.id
        )
        TournamentParticipant.deck_is_from_participant_validator(
            player=player, deck=deck
        )
        TournamentParticipant.inscription_in_time_validator(
            tournament=tournament, inscription_date=inscription_date
        )

    def add_participant(tournament, player, deck, inscription_date):
        TournamentParticipant.check_validations(
            tournament, player, deck, inscription_date
        )
        element = TournamentParticipant(
...
class Duel(models.Model):
    ...
    player1 = models.ForeignKey(
        Player, on_delete=models.CASCADE, related_name="was_player1"
    )
    player2 = models.ForeignKey(
        Player, on_delete=models.CASCADE, related_name="was_player2"
    )
    tournament = models.ForeignKey(
        Tournament, on_delete=models.CASCADE, related_name="duels"
    )
